    ...
    <Card className="hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1 dark:bg-slate-800 dark:border-slate-700">
      ...
      <CardContent>
        <div className="space-y-6">
          {data.map((item, index) => (
            <div key={index} className="flex items-start space-x-4 p-4 rounded-lg bg-gray-50 dark:bg-slate-700/50">
              <div className={`p-3 rounded-lg transition-colors duration-300 ${getColorClasses(item.color)}`}>
                {item.icon}
              </div>
